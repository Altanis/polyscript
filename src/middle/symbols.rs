...
impl SemanticAnalyzer {
    ...
    fn symbol_collector_check_node(&mut self, node: &mut AstNode) -> Result<(Option<ValueSymbolId>, Option<Type>), BoxedError> {
        ...
        let declared_symbol_opt = match &mut node.kind {
            ...
            TraitDeclaration { name, generic_parameters, constants, types, signatures } =>
            ...
            Block(_) =>
                self.collect_block_symbols(node),
            _ => {
    ...
    fn collect_function_item_symbols(&mut self, node: &mut AstNode) -> Result<(Option<ValueSymbolId>, Option<Type>), BoxedError> {
        let (is_declaration, qualifier, name, generic_parameters, parameters, return_type, body, instance) =
            if let AstNodeKind::Function { qualifier, name, generic_parameters, parameters, return_type, body, instance } = &mut node.kind {
                (!name.is_empty(), *qualifier, name.clone(), generic_parameters, parameters, return_type, body, instance)
            } else { 
                unreachable!(); 
            };
    ...
    fn collect_trait_symbols(
        ...
        let trait_type_id = self.symbol_table.add_type_symbol(name, TypeSymbolKind::Trait(trait_scope_id), generic_param_ids, QualifierKind::Public, Some(span))?;
    ...
    fn collect_block_symbols(&mut self, node: &mut AstNode) -> Result<(Option<ValueSymbolId>, Option<Type>), BoxedError> {
...
impl SemanticAnalyzer {
    ...
    fn collect_impl_body_symbols(
        ...
        for func_node in associated_functions {
            if let AstNodeKind::Function { qualifier, name, generic_parameters, parameters, body, instance, .. } = &mut func_node.kind {
