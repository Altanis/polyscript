    ...
    fn apply_substitution(ty: &Type, substitutions: &HashMap<TypeSymbolId, Type>) -> Type {
        match ty {
                ...
                if let Some(substituted_type) = substitutions.get(symbol) {
                    ...
                    } = substituted_type
                    {
                        if new_args.is_empty() {
                            let final_args = args
                                .iter()
                                .map(|arg| SemanticAnalyzer::apply_substitution(arg, substitutions))
                                .collect();
                            return Type::Base {
                                symbol: *new_symbol,
                                args: final_args,
            ...
            Type::Reference(inner) => {
                Type::Reference(Box::new(SemanticAnalyzer::apply_substitution(inner, substitutions)))
            }
            Type::MutableReference(inner) => Type::MutableReference(Box::new(
                SemanticAnalyzer::apply_substitution(inner, substitutions),
            )),
        }
    }
...
impl SemanticAnalyzer {
    ...
    fn unify_static_member_access(
        ...
        let lhs_type_id = substituted_lhs.get_base_symbol();
