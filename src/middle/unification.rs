...
impl SemanticAnalyzer {
    ...
    fn unify_variable(&mut self, uv_id: TypeSymbolId, ty: Type) {
        let TypeSymbolKind::UnificationVariable(id) = self.symbol_table.get_type_symbol(uv_id).unwrap().kind else {
            unreachable!();
        };
    ...
    fn is_uv(&self, symbol_id: TypeSymbolId) -> bool {
        matches!(self.symbol_table.get_type_symbol(symbol_id).unwrap().kind, TypeSymbolKind::UnificationVariable(_))
    }
    ...
    fn type_mismatch_error(&self, t1: &Type, t2: &Type) -> Result<(), BoxedError> {
        ...
        Err(self.create_error(
            ErrorKind::TypeMismatch(self.symbol_table.display_type(t1), self.symbol_table.display_type(t2)),
            span1,
            &[span1, span2],
        ))
    }
