...
impl<'a, 'ctx> CodeGen<'a, 'ctx> {
    ...
    pub fn build_free(&self, ptr: PointerValue<'ctx>) {
        ...
        // Add a helper to get the printf function
    ...
    // Add a method to generate a call to the specialized print function
    fn compile_print_call(&mut self, value: BasicValueEnum<'ctx>, ty: &Type) {
        let print_fn = self.get_or_create_print_fn(ty);
        self.builder.build_call(print_fn, &[value.into()], "print_call").unwrap();
    }
    
    // Add a method to get or create the specialized print function
    fn get_or_create_print_fn(&mut self, ty: &Type) -> FunctionValue<'ctx> {
        let mangled_name = format!("__print_{}", self.mangle_name(ty));
        ...
        let function = self.module.add_function(&mangled_name, fn_type, Some(Linkage::Internal));

        let old_block = self.builder.get_insert_block();
        let entry = self.context.append_basic_block(function, "entry");
        self.builder.position_at_end(entry);

        let param_value = function.get_first_param().unwrap();
        self.build_print_fn_body(param_value, ty, function);
    ...
    // Add the core logic for generating the print function's body
    fn build_print_fn_body(&mut self, value: BasicValueEnum<'ctx>, ty: &Type, function: FunctionValue<'ctx>) {
        ...
        match ty {
            Type::Base { symbol, .. } => {
                ...
                match &type_symbol.kind {
                    TypeSymbolKind::Primitive(p) => {
                        let (format_str, val_to_print) = match p {
                            PrimitiveKind::Int => ("%lld\n", value),
                            ...
                            PrimitiveKind::Bool => {
                                ...
                                let str_val = self.builder.build_select(value.into_int_value(), true_str.as_pointer_value(), false_str.as_pointer_value(), "bool_str").unwrap();
                                ("%s", str_val.into())
                            },
                            PrimitiveKind::Char => ("%c\n", value),
                            PrimitiveKind::StaticString => ("%s\n", value),
                            _ => return, // void, never
                        };
                        let format_str_ptr = self.builder.build_global_string_ptr(format_str, "format_str").unwrap();
                        self.builder.build_call(printf, &[format_str_ptr.as_pointer_value().into(), val_to_print.into()], "printf").unwrap();
                    },
                    TypeSymbolKind::Struct((scope_id, _)) => {
                        ...
                        let intro_format = self.builder.build_global_string_ptr(&format!("{} {{ ", struct_name), "intro_fmt").unwrap();
                        self.builder.build_call(printf, &[intro_format.as_pointer_value().into()], "printf").unwrap();
                        ...
                        for (i, field_symbol) in field_symbols.iter().enumerate() {
                            ...
                            let field_name_format = self.builder.build_global_string_ptr(&format!("{}: ", field_name), "field_name_fmt").unwrap();
                            self.builder.build_call(printf, &[field_name_format.as_pointer_value().into()], "printf").unwrap();

                            let field_value = self.builder.build_extract_value(struct_val, i as u32, "field_val").unwrap();
                            let field_print_fn = self.get_or_create_print_fn(field_type);
                            self.builder.build_call(field_print_fn, &[field_value.into()], "print_field").unwrap();
                            
                            if i < field_symbols.len() - 1 {
                                let comma_format = self.builder.build_global_string_ptr(", ", "comma_fmt").unwrap();
                                self.builder.build_call(printf, &[comma_format.as_pointer_value().into()], "printf").unwrap();
                            }
                        }

                        let outro_format = self.builder.build_global_string_ptr(" }\n", "outro_fmt").unwrap();
                        self.builder.build_call(printf, &[outro_format.as_pointer_value().into()], "printf").unwrap();
                    },
                    TypeSymbolKind::Enum(_) => {
                        let format_str_ptr = self.builder.build_global_string_ptr("%lld\n", "format_str").unwrap();
                        self.builder.build_call(printf, &[format_str_ptr.as_pointer_value().into(), value.into()], "printf").unwrap();
                    },
                    _ => {
                        let format_str_ptr = self.builder.build_global_string_ptr(&format!("<unprintable type: {}>\n", self.analyzer.symbol_table.display_type(ty)), "format_str").unwrap();
                        self.builder.build_call(printf, &[format_str_ptr.as_pointer_value().into()], "printf").unwrap();
                    }
                }
            },
            Type::Reference { inner, .. } | Type::MutableReference { inner, .. } => {
                ...
                let null_block = self.context.append_basic_block(function, "null");
                let end_block = self.context.append_basic_block(function, "end_print_ref");
                self.builder.build_conditional_branch(is_null, null_block, not_null_block).unwrap();

                self.builder.position_at_end(null_block);
                let null_str = self.builder.build_global_string_ptr("null\n", "null_str").unwrap();
                self.builder.build_call(printf, &[null_str.as_pointer_value().into()], "printf_null").unwrap();
                self.builder.build_unconditional_branch(end_block).unwrap();

                self.builder.position_at_end(not_null_block);
                let inner_llvm_type = self.map_semantic_type(inner).unwrap();
                let inner_val = self.builder.build_load(inner_llvm_type, ptr_val, "deref").unwrap();
                let inner_print_fn = self.get_or_create_print_fn(inner);
                self.builder.build_call(inner_print_fn, &[inner_val.into()], "print_inner").unwrap();
                self.builder.build_unconditional_branch(end_block).unwrap();
    ...
    fn compile_node(&mut self, stmt: &MIRNode) -> Option<BasicValueEnum<'ctx>> {
        match &stmt.kind {
            ...
            MIRNodeKind::PrintStatement(expr) => {
                let value_to_print = self.compile_node(expr)?;
                let value_type = expr.type_id.as_ref().unwrap();
                self.compile_print_call(value_to_print, value_type);
                None
            },
