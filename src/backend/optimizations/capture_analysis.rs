use crate::{
    frontend::semantics::analyzer::{SemanticAnalyzer, TypeSymbolKind},
    ...
    let current_node_usage = match &node.kind {
        ...
        MIRNodeKind::FunctionCall { function, arguments } => {
            ...
            if let Some(fn_type) = &function.type_id {
                let fn_type_symbol = analyzer.symbol_table.get_type_symbol(fn_type.symbol).unwrap();
                if let TypeSymbolKind::FunctionSignature { params, .. } = &fn_type_symbol.kind {
                    for (i, arg_node) in arguments.iter().enumerate() {
                        if get_base_variable(arg_node) == Some(target_id) && params.get(i).is_some() {
                            let usage_from_param = UsageKind::ByValue;
